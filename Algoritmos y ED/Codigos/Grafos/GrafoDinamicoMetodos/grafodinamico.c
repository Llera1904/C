...
{
	GrafoDinamico q= NULL;
	Nodo aux= NULL, sig= NULL, enlace= NULL;
	int sw= FALSO;
	int nE=0, nS=0;
	int gradSal=0;
	...
	while(aux!=NULL)
	{
		...
		consultarGradoSalida(aux, &gradSal);
		(q->numAristas)-= gradSal;
		
		if (sw==FALSO)
		  return(FALSO);
		  
		//aux= aux->enlace;
		consultarEnlace(&enlace, aux);
		aux= enlace;
	}
...
int buscarNodo(GrafoDinamico g, Nodo *n, int id)
{
	...
	while(aux!=NULL)
	{
		if (compararID(aux, id)==VERDADERO)
		  {
		  	*n= aux;
		  	return(VERDADERO);
		  }
...
int generarArbolAmplitud(GrafoDinamico *arbol, GrafoDinamico g, int id)
{
	...
	Nodo aux= NULL, raiz= NULL, enlace= NULL;
	Elemento e= NULL;
	int *visitados= NULL;
	int sw= FALSO;
	int k=0;
...
int recorrerGrafoAmplitud(GrafoDinamico arb, GrafoDinamico g, Nodo centro, int id, int vecindario) // Nodo vecindario // cola
{
	...
	int k=0, sw= FALSO, avance= NOVISITADO;
	int idFte=-1, idDest=-1;
	
	aux= g->inicial;
	
	while (aux!=NULL)
	{		  
		consultarProfundidad(aux, &avance);
	  
		if (avance==vecindario)
		  {	
			...
			while(arista!=NULL)
			{
				...
				consultarSalida(&destino, arista);
				
				consultarProfundidad(destino, &avance);
				if (avance==NOVISITADO)
		  		  {
		  		  	consultarID(fuente, &idFte);
		  		  	consultarID(destino, &idDest);
		  		  	
		  			sw= agregarConexion(arb, idFte, idDest, vecindario+1);
		  			if (sw==FALSO)
		  	  		  return(FALSO);
		  	  
		  			// destino->profundidad= vecindario+1;
		  			establecerProfundidad(destino, vecindario+1);
		  			descendientes= VERDADERO;
		  		  }

				// arista= arista->enlace;
				consultarEnlace(&enlace, arista);
				arista= enlace;
			}
		  }
	...
	if (descendientes==VERDADERO)
		{
			sw= recorrerGrafoAmplitud(arb, g, destino, idDest, vecindario+1);
		  	if (sw==FALSO)
		  	  return(FALSO);
		}
...
int recorrerGrafoProfundidad(GrafoDinamico arb, GrafoDinamico g, Nodo centro, int id, int nivel) // Nodo vecindario // cola
{
	Nodo arista= NULL, fuente= NULL, destino= NULL;
	Nodo enlace= NULL;
	int descendientes= FALSO, avance= NOVISITADO;
	int k=0, sw= FALSO;
	int idFte= -1, idDest= -1;
	
	consultarProfundidad(centro, &avance);
	if (avance==NOVISITADO)
	  establecerProfundidad(centro, nivel);
		...
		if (avance==NOVISITADO)
		  {
		  	...
		  	sw= agregarConexion(arb, idFte, idDest, nivel+1);
		  	if (sw==FALSO)
		  	  return(FALSO);
		  			  
		  	//destino->profundidad= nivel+1;
		  	establecerProfundidad(destino, nivel+1);
...
{
	Nodo vertice= NULL, arista= NULL, enlace= NULL;
	Nodo fuente= NULL, destino= NULL;
	...
	while(vertice!=NULL)
	{
		...
		consultarSalida(&arista, vertice);
		...
		while(arista!=NULL)
		{
			...
			consultarSalida(&destino, arista);
	   		...
	   		consultarPeso(arista, &peso);
	   		   		  	
	   		printf(" <%d, %d> : %d |", datoA, datoB, peso);
		...
		consultarEnlace(&enlace, vertice);
		vertice= enlace;
